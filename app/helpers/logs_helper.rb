      ...
      if !log.target_type.blank? && !log.target_id.blank? # make sure there's a target class and id
        if log.target # does target actually  
          if log.target_type == "Item"
            message = I18n.t("log.models.#{log.target_type.underscore}.#{log.log_type}", :user => link_to_user(log.target.user), :item => log.klass.model_name.human.downcase, :target => link_to_item(log.target))        
          elsif log.target_type == "PluginComment"    
            message = I18n.t("log.models.#{log.target_type.underscore}.#{log.log_type}", :user => link_to_user(log.target.user), :item => log.klass.model_name.human.downcase, :parent => link_to_item(log.target.item), :target => link_to(truncate(log.target.comment, :length => 40), {:action => "view", :controller => "items", :anchor => log.klass.model_name.human(:count => :other)}))
          elsif log.target_type == "PluginImage"    
            message = thumbnail(log.target, :class => "medium", :preview => true, :style => "float:left;margin-right:5px;")
            message += raw(I18n.t("log.models.#{log.target_type.underscore}.#{log.log_type}", :user => link_to_user(log.target.user), :item => log.klass.model_name.human.downcase, :parent => link_to_item(log.target.item), :target => link_to_item(log.target.item)))
          elsif log.target_type == "PluginReview"    
            message = I18n.t("log.models.#{log.target_type.underscore}.#{log.log_type}", :user => link_to_user(log.target.user), :item => log.klass.model_name.human.downcase, :parent => link_to_item(log.target.item), :target => link_to(log.target.item.name, {:action => "show", :controller => "plugin_reviews", :id => log.target.item_id, :review_id => log.target_id}))
          elsif log.target_type == "PluginDiscussionPost"
            message = I18n.t("log.models.#{log.target_type.underscore}.#{log.log_type}", :user => link_to_user(log.target.user), :item => log.klass.model_name.human.downcase, :parent => link_to_item(log.target.plugin_discussion.item), :discussion => link_to(log.target.plugin_discussion.title, {:action => "view", :controller => "plugin_discussions", :id => log.target.plugin_discussion.item, :discussion => log.target.plugin_discussion}), :target => link_to(truncate(log.target.post, :length => 20), {:action => "view", :controller => "plugin_discussions", :id => log.target.plugin_discussion.item.id, :discussion_id => log.target.plugin_discussion, :anchor => log.target_id}))
          elsif log.target_type == "PluginDiscussion"
            message = I18n.t("log.models.#{log.target_type.underscore}.#{log.log_type}", :user => link_to_user(log.target.user), :item => log.klass.model_name.human.downcase, :parent => link_to_item(log.target.item), :target => link_to(log.target.title, {:action => "view", :controller => "plugin_discussions", :id => log.target.item_id, :discussion_id => log.target}))          
