    ...
    if log.log # static log message?
      ...
      if !log.target_type.blank? && !log.target_id.blank? # make sure there's a target class and id
        if log.target # does target actually exist? 
          ...
          when "PluginReview"    
            options = {:user => link_to_user(log.target.user), :item => log.klass.model_name.human.downcase, :parent => link_to_record(log.target.record), :target => link_to(log.target.record.name, {:action => "show", :controller => "plugin_reviews", :id => log.target_id})}
          when "PluginDiscussionPost"
            options = {:user => link_to_user(log.target.user), :item => log.klass.model_name.human.downcase, :parent => link_to_record(log.target.plugin_discussion.record), :discussion => link_to(log.target.plugin_discussion.title, {:action => "view", :controller => "plugin_discussions", :id => log.target.plugin_discussion.record, :discussion => log.target.plugin_discussion}), :target => link_to(truncate(log.target.post, :length => 20), {:action => "view", :controller => "plugin_discussions", :id => log.target.plugin_discussion.record.id, :discussion_id => log.target.plugin_discussion, :anchor => log.target_id})}
          when "PluginDiscussion"
            options = {:user => link_to_user(log.target.user), :item => log.klass.model_name.human.downcase, :parent => link_to_record(log.target.record), :target => link_to(log.target.title, {:action => "view", :controller => "plugin_discussions", :id => log.target.record_id, :discussion_id => log.target})}          
          when "Page"
