      ...
      if !log.target_type.blank? && !log.target_id.blank? # make sure there's a target class and id
        if log.target # does target actually exist? 
          ...
          when "PluginDiscussionPost"
            options = {:user => link_to_user(log.target.user), :item => log.klass.model_name.human.downcase, :parent => link_to_record(log.target.plugin_discussion.record), :discussion => link_to(log.target.plugin_discussion.title, {:action => "view", :controller => "plugin_discussions", :record_id => log.target.plugin_discussion, :record_type => log.target.plugin_discussion.class.name}), :target => link_to(truncate(log.target.post, :length => 20), {:action => "view", :controller => "plugin_discussions", :record_type => log.target.plugin_discussion.record.class.name, :record_id => log.target.plugin_discussion, :anchor => log.target_id})}
