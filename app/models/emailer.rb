class Emailer < ActionMailer::Base
  ...
  def verification_email(user_verification)
    @setting = Setting.global_settings
    @user_verification = user_verification
    subject = I18n.t("email.subject.verification", :name => @user_verification.user.to_s, :title => @setting[:title])
    mail(:to => nil, :bcc => recipients, :subject =>  subject, :date => Time.now)
  end
  ...
  def new_plugin_record_notification(record)
    recipients = record.record.user.email
    @record = record
